...
import { deleteTrainingSessionDraft } from "../../lib/api";
      ...
      return [
        ...
        downloadAction,
        {
          ...
          onClick: async () => {
            ...
            if (currentUser?.id && Number.isFinite(sessionId) && sessionId > 0) {
              try {
                await deleteTrainingSessionDraft(sessionId, currentUser.id);
              } catch {
                window.alert("Unable to delete this draft from the server. Please try again.");
                return;
              }
            }
            removeForm(form.id);
