...
public class TrainerReportService : ITrainerReportService
{
    ...
    public async Task SubmitSupervisorSignoffAsync(int sessionId, SubmitSupervisorSignoffDto dto)
    {
        var report = await _context.TrainerReports
            .Include(r => r.TrainingSession)
            .FirstOrDefaultAsync(r => r.TrainingSessionId == sessionId);

        if (report is null)
            throw new Exception("Report not found");

        if (!report.SubmittedToSupervisor)
            throw new Exception("Trainer has not submitted this assessment for supervisor review.");

        var session = report.TrainingSession;
        var isApproval = dto.Decision.Equals("Approve", StringComparison.OrdinalIgnoreCase);

        report.SupervisorComment = dto.Comments;
        ...
        if (isApproval)
        {
            ...
            report.SupervisorSignature = dto.SupervisorSignature;
            report.SupervisorDate = DateTime.UtcNow;
            report.SupervisorSignatureStatus = SignatureStatus.SIGNED;
            session.Status = AssessmentStatus.COMPLETED;
        }
        else
        {
            report.SupervisorName = dto.SupervisorName;
            report.SupervisorSignature = string.Empty;
            report.SupervisorDate = null;
            report.SupervisorSignatureStatus = SignatureStatus.READYTOSIGN;
            report.SubmittedToSupervisor = false;
