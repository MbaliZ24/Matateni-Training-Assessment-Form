...
public class TrainingService : ITrainingService
{
    ...
    public async Task<List<TrainingSessionSummaryDto>> GetSupervisorSessionsAsync(string supervisorId)
    {
        var sessions = await _context.TrainingSessions
            ...
            .Include(x => x.TrainerReport)
            .Where(x =>
                x.AssignedSupervisorId == supervisorId &&
                x.TrainerReport != null &&
                (
                    x.TrainerReport.SubmittedToSupervisor ||
                    x.Status == AssessmentStatus.COMPLETED ||
                    x.Status == AssessmentStatus.FOLLOWUPPENDING
                ))
    ...
    public async Task<List<TrainingSessionSummaryDto>> GetAllSessionsAsync()
    {
        var sessions = await _context.TrainingSessions
            .Include(x => x.FeedbackSubmissions)
                .ThenInclude(x => x.Answers)
    ...
    private TrainingSessionSummaryDto MapSessionSummary(TrainingSession session)
    {
        ...
        var report = session.TrainerReport;
        string? supervisorDecision = null;

        if (report is not null)
        {
            if (report.SupervisorSignatureStatus == SignatureStatus.SIGNED && session.Status == AssessmentStatus.COMPLETED)
            {
                supervisorDecision = "Approve";
            }
            else if (
                !report.SubmittedToSupervisor &&
                session.Status == AssessmentStatus.FOLLOWUPPENDING &&
                !string.IsNullOrWhiteSpace(report.SupervisorComment))
            {
                supervisorDecision = "Needs Changes";
            }
        }
        ...
        return new TrainingSessionSummaryDto
        {
            ...
            SupervisorReviewDecision = supervisorDecision,
            SupervisorReviewComments = report?.SupervisorComment,
            SupervisorSignedBy = report?.SupervisorName,
            SupervisorSignedAt = report?.SupervisorDate
        };
    }
