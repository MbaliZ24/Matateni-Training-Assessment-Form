...
export function TrainerFeedbackPage() {
  ...
  const feedbackForms = useMemo(
    () =>
      forms
        .filter((f) => f.trainerId === currentUser?.id && !!f.supervisorReview)
        .sort((a, b) => (b.supervisorReview?.updatedAt ?? "").localeCompare(a.supervisorReview?.updatedAt ?? "")),
                    ...
                    <div className="flex items-start justify-between gap-2">
                      <div>
                        ...
                        <p className="text-xs text-slate-500">
                          {form.supervisorReview?.decision} • {(form.supervisorReview?.updatedAt ?? "").slice(0, 10)}
                        </p>
                      </div>
              ...
              <div className="space-y-4 text-sm">
                <div className="grid gap-3 md:grid-cols-3">
                  <div className="rounded-lg border border-slate-200 p-3">
                    ...
                    <p className="mt-1 font-semibold text-slate-900">{selected.supervisorReview.decision}</p>
                  </div>
                  <div className="rounded-lg border border-slate-200 p-3">
                ...
                <div className="rounded-lg border border-slate-200 p-3">
                  ...
                  <p className="mt-1 text-slate-800">{selected.supervisorReview.comments || "No comments."}</p>
                </div>

                <div className="rounded-lg border border-slate-200 p-3">
                  <p className="text-xs uppercase tracking-wide text-slate-500">Action items</p>
                  {selected.supervisorReview.actionItems.length === 0 ? (
                ...
                <div className="rounded-lg border border-slate-200 p-3">
                  ...
                  <ul className="mt-2 space-y-1 text-slate-700">
                    ...
                    <li>
                      • Supervisor {selected.supervisorReview.stage === "draft" ? "saved draft" : "submitted feedback"} ({selected.supervisorReview.updatedAt.slice(0, 10)})
                    </li>
